...
| [garrytan/gbrain](https://github.com/garrytan/gbrain) | (see upstream) | Comparative pattern reference for evidence-envelope fields, frozen additive-forever agent protocol versioning, and budgeted remediation. Informed the shape of the private monorepo's Organizer MCP protocol; no gbrain code copied. Promoted to TR-AGT-010 in v0.12.0 once live-mount conformance tests existed in the private monorepo, per this row's original gate. |
...
| [koala73/worldmonitor](https://github.com/koala73/worldmonitor) | AGPL-3.0 | Comparative pattern reference only: SSRF allowlist + redirect-hop re-check with honest residuals informed `examples/ssrf-allowlist/` (MIT reimplementation — no upstream code). Graded source confidence and hybrid classify-with-source were privately dogfooded and promoted to TR-SEC-015/TR-AGT-011 in v0.12.0 (see `CHANGELOG.md`); LLM call dedup remains citation-only, still unpromoted. Adoption decisions live in the private monorepo (ADR-017). |
